// import React, { useState, useEffect } from 'react';

// const ButtonAddToBookmarks = ({ articleId }) => {
//   const [isSaved, setIsSaved] = useState(false);
//   const [loading, setLoading] = useState(false);

//   useEffect(() => {
//     // Приклад запиту для перевірки стану на бекенді
//     // fetch(`/api/bookmarks/check?articleId=${articleId}`)
//     //   .then(response => response.json())
//     //   .then(data => setIsSaved(data.isSaved))
//     //   .catch(error => console.error('Error checking bookmark:', error));
//   }, [articleId]);

//   const handleClick = async () => {
//     if (loading) return;

//     setLoading(true);
//     try {
//       const response = await fetch('/api/bookmarks', {
//         method: isSaved ? 'DELETE' : 'POST',
//         headers: {
//           'Content-Type': 'application/json',
//         },
//         body: JSON.stringify({ articleId }),
//       });

//       if (!response.ok) {
//         throw new Error('Failed to save bookmark');
//       }

      
//       setIsSaved(prev => !prev);
//     } catch (error) {
//       console.error('Error saving bookmark:', error);
//     } finally {
//       setLoading(false);
//     }
//   };

//   return (
//     <button
//       className="button-add-to-bookmarks"
//       onClick={handleClick}
//       disabled={loading}
//       style={{
//         border: isSaved ? 'none' : '1px solid #374F42',
//         background: isSaved ? '#649179' : '#D1E0D8',
//         width: '40px',
//         height: '40px',
//         borderRadius: '64px',
//         opacity: loading ? 0.6 : 1,
//         gap: '8px',
//         padding: '8px',
//         cursor: loading ? 'not-allowed' : 'pointer',
//         display: 'flex',
//         alignItems: 'center',
//         justifyContent: 'center',
//       }}
//     >
//       <svg width="24" height="24" viewBox="0 0 26 32">
//         <path
//           strokeLinejoin="round"
//           strokeLinecap="round"
//           strokeMiterlimit="4"
//           strokeWidth="1.8824"
//           fill="none"
//           stroke={isSaved ? '#FFFFFF' : '#374F42'}
//           d="M13.171 0.941c2.379 0 4.562 0.228 6.292 0.491 2.191 0.333 3.908 1.834 4.397 3.932 0.59 2.532 1.169 6.385 1.070 11.559-0.11 5.709-0.943 9.829-1.746 12.516-0.217 0.727-0.72 1.101-1.289 1.197-0.594 0.1-1.328-0.098-1.925-0.691-1.086-1.081-2.329-2.248-3.476-3.151-0.572-0.45-1.142-0.852-1.671-1.145-0.499-0.277-1.083-0.528-1.653-0.528-0.56 0-1.165 0.247-1.693 0.518-0.563 0.289-1.186 0.686-1.824 1.136-1.277 0.902-2.692 2.070-3.938 3.153-0.658 0.571-1.432 0.706-2.042 0.54-0.589-0.16-1.086-0.613-1.239-1.401-0.525-2.707-1.024-6.705-1.024-12.127 0-5.409 0.557-9.224 1.112-11.68 0.46-2.035 2.12-3.485 4.255-3.814 1.746-0.27 3.967-0.505 6.392-0.505z"
//         />
//       </svg>
//     </button>
//   );
// };

// export default ButtonAddToBookmarks;

import React, { useState, useEffect } from 'react';
import { toast } from 'react-toastify'; // Для пуш-повідомлень, встановіть: npm install react-toastify
import 'react-toastify/dist/ReactToastify.css'; // Імпорт стилів для toast

// Приклад компонента модального вікна (замініть на ваш реальний компонент)
const ErrorSaveModal = ({ onClose }) => (
  
);

const ButtonAddToBookmarks = ({ articleId }) => {
  const [isSaved, setIsSaved] = useState(false);
  const [loading, setLoading] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isAuthenticated, setIsAuthenticated] = useState(false);

  // Імітація перевірки авторизації (замініть на реальну логіку)
  useEffect(() => {
    // Перевірка авторизації (наприклад, з localStorage або контексту)
    const token = localStorage.getItem('token'); // Приклад
    setIsAuthenticated(!!token);

    // Ініціалізація стану з бекенду (опціонально)
    // fetch(`/api/bookmarks/check?articleId=${articleId}`, { headers: { Authorization: `Bearer ${token}` } })
    //   .then(response => response.json())
    //   .then(data => setIsSaved(data.isSaved))
    //   .catch(error => console.error('Error checking bookmark:', error));
  }, [articleId]);

  const handleClick = async () => {
    if (loading) return;

    if (!isAuthenticated) {
      setIsModalOpen(true);
      return;
    }

    setLoading(true);
    try {
      const response = await fetch('/api/bookmarks', {
        method: isSaved ? 'DELETE' : 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
        body: JSON.stringify({ articleId }),
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to save bookmark');
      }

      setIsSaved(prev => !prev); 
    } catch (error) {
      toast.error(`Помилка: ${error.message}`, {
        position: 'top-right',
        autoClose: 5000,
      });
    } finally {
      setLoading(false);
    }
  };

  const closeModal = () => {
    setIsModalOpen(false);
  };

  return (
    <>
      <button
        className="button-add-to-bookmarks"
        onClick={handleClick}
        disabled={loading}
        style={{
          border: isSaved ? 'none' : '1px solid #374F42',
          background: isSaved ? '#649179' : '#D1E0D8',
          width: '40px',
          height: '40px',
          borderRadius: '64px',
          opacity: loading ? 0.6 : 1,
          gap: '8px',
          padding: '8px',
          cursor: loading ? 'not-allowed' : 'pointer',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
        }}
      >
        <svg width="24" height="24" viewBox="0 0 26 32">
          <path
            strokeLinejoin="round"
            strokeLinecap="round"
            strokeMiterlimit="4"
            strokeWidth="1.8824"
            fill="none"
            stroke={isSaved ? '#FFFFFF' : '#374F42'}
            d="M13.171 0.941c2.379 0 4.562 0.228 6.292 0.491 2.191 0.333 3.908 1.834 4.397 3.932 0.59 2.532 1.169 6.385 1.070 11.559-0.11 5.709-0.943 9.829-1.746 12.516-0.217 0.727-0.72 1.101-1.289 1.197-0.594 0.1-1.328-0.098-1.925-0.691-1.086-1.081-2.329-2.248-3.476-3.151-0.572-0.45-1.142-0.852-1.671-1.145-0.499-0.277-1.083-0.528-1.653-0.528-0.56 0-1.165 0.247-1.693 0.518-0.563 0.289-1.186 0.686-1.824 1.136-1.277 0.902-2.692 2.070-3.938 3.153-0.658 0.571-1.432 0.706-2.042 0.54-0.589-0.16-1.086-0.613-1.239-1.401-0.525-2.707-1.024-6.705-1.024-12.127 0-5.409 0.557-9.224 1.112-11.68 0.46-2.035 2.12-3.485 4.255-3.814 1.746-0.27 3.967-0.505 6.392-0.505z"
          />
        </svg>
      </button>

      {isModalOpen && <ErrorSaveModal onClose={closeModal} />}
    </>
  );
};

export default ButtonAddToBookmarks;