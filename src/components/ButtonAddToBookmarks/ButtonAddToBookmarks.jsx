...
const ButtonAddToBookmarks = ({
  ...
  const savedArticles = rawSavedArticles;
  ...
  const toggleBookmark = async () => {
    if (!articleId) {
      ...
      console.error(
        'ButtonAddToBookmarks - toggleBookmark - Invalid articleId:',
        articleId
      );
      return;
    }
    ...
    try {
      ...
      if (isSaved) {
        const result = await dispatch(
          removeArticleFromBookmarks(String(articleId))
        ).unwrap();
        ...
        const result = await dispatch(
          addArticleToBookmarks(String(articleId))
        ).unwrap();
      ...
      console.error(
        'ButtonAddToBookmarks - toggleBookmark - Error:',
    ...
    <>
      ...
      >
        {loading ? (
          ...
          <>
            {isWideStyle && (
              <span className={s.text}>{isSaved ? 'Unsave' : 'Save'}</span>
            )}
            <svg
